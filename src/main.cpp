...
#if defined(NDEBUG)
#error "ONEX cannot be compiled without assertions."
#endif
...
int64_t nReserveBalance = 500 * COIN;

/** Fees smaller than this (in uONEX) are considered zero fee (for relaying and mining)
...
int64_t GetBlockValue(int nHeight) {
    ...
    if (nHeight == 0) {
        ...
        nSubsidy = 6 * COIN;
    } else if (nHeight < 320000 && nHeight >= 200000) {
        nSubsidy = 7 * COIN;
    } else if (nHeight < 440000 && nHeight >= 320000) {
        nSubsidy = 6 * COIN;
    } else if (nHeight < 560000 && nHeight >= 440000) {
        nSubsidy = 5 * COIN;
    } else if (nHeight < 680000 && nHeight >= 560000) {
        nSubsidy = 4 * COIN;
    } else if (nHeight < 800000 && nHeight >= 680000) {
        nSubsidy = 3 * COIN;
...
bool IsInitialBlockDownload() {
    ...
    bool state = (chainActive.Height() < pindexBestHeader->nHeight - 24 * 6 ||
                 ...
                 chainActive.Height() == 58206; // ~144 blocks behind -> 2 x fork detection time
    if (!state)
        lockIBDState = true;
    return state;
}
...
bool CheckBlock(const CBlock &block, CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) {
    ...
    if (block.IsProofOfStake()) {
        ...
        if (txPrev.vout[block.vtx[1].vin[0].prevout.n].nValue < Params().StakeInputMinimal()) {
            printf("%lld < %lld \n", txPrev.vout[block.vtx[1].vin[0].prevout.n].nValue, Params().StakeInputMinimal());
...
bool static ProcessMessage(CNode *pfrom, string strCommand, CDataStream &vRecv, int64_t nTimeReceived) {
    ...
    if (strCommand == "version") {
        ...
        // ONEX: We use certain sporks during IBD, so check to see if they are
