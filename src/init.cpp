...
std::string HelpMessage(HelpMessageMode mode) {
    ...
    strUsage += HelpMessageOpt("-checklevel=<n>",
                               strprintf(_("How thorough the block verification of -checkblocks is (0-4, default: %u)"),
                                         3));
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "ONEX.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "ONEXd.pid"));
#endif
    ...
    strUsage += HelpMessageOpt("-port=<port>",
                               strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 30229,
                                         130229));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in ONEX/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in ONEX/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, ONEX, (swifttx, masternode, mnpayments, mnbudget)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(
            _("Fees (in ONEX/Kb) smaller than this are considered zero fee for relaying (default: %s)"),
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(
            _("Disable all ONEX specific functionality (Masternodes, SwiftTX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-enableswifttx=<n>", strprintf(
            _("Enable swifttx, show confirmations for locked transactions (bool, default: %s)"), "true"));
    ...
    strUsage += HelpMessageOpt("-rpcthreads=<n>",
                               strprintf(_("Set the number of threads to service RPC calls (default: %d)"), 4));
    strUsage += HelpMessageOpt("-rpckeepalive",
                               strprintf(_("RPC support for HTTP persistent connections (default: %d)"), 1));

    strUsage += HelpMessageGroup(_("RPC SSL options: (see the Bitcoin Wiki for SSL setup instructions)"));
    strUsage += HelpMessageOpt("-rpcssl", _("Use OpenSSL (https) for JSON-RPC connections"));
    strUsage += HelpMessageOpt("-rpcsslcertificatechainfile=<file.cert>",
                               strprintf(_("Server certificate file (default: %s)"), "server.cert"));
    strUsage += HelpMessageOpt("-rpcsslprivatekeyfile=<file.pem>",
                               strprintf(_("Server private key (default: %s)"), "server.pem"));
    strUsage += HelpMessageOpt("-rpcsslciphers=<ciphers>", strprintf(_("Acceptable ciphers (default: %s)"),
...
/** Sanity checks
 *  Ensure that ONEX is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
...
#endif
    // Make sure only a single ONEX process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(
                strprintf(_("Cannot obtain a lock on data directory %s. ONEX Core is probably already running."),
                          strDataDir));
    ...
    {
        printf("%s", GetArg("-sporkkey", "").c_str());
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                // ONEX: load previous sessions sporks if we have them.
